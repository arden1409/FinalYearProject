...
namespace GifImporter.Editors
{
    public class GifImporter : AssetPostprocessor
    {
        ...
        private static void GenerateSprites(string file)
        {
            ...
            List<GifExtract.Core.Frame> framesData = null;
            try
            {
                framesData = Core.GetFrames(filePath);
            }
            catch (Exception ex)
            {
                Debug.LogError($"[GifImporter] Failed to decode GIF {file}. {ex.Message}");
                return;
            }

            if (framesData == null || framesData.Count == 0)
            {
                Debug.LogError($"[GifImporter] GIF {file} returned no frames.");
                return;
            }
